...
fn gen_explosion_sound() -> Vec<u8> {
    ...
    for i in 0..n {
        ...
        let freq = 50.0 - t * 20.0;
        let rumble = (t * freq * std::f32::consts::TAU).sin() * 0.5;
        let rumble2 = (t * freq * 1.5 * std::f32::consts::TAU).sin() * 0.3;
        let noise = ((i as f32 * 3456.789).sin() * 43758.5453).fract() * 2.0 - 1.0;
        let crackle = if i < 2000 { noise * 0.7 } else { noise * 0.2 };
        s[i] = ((rumble + rumble2 + crackle) * env * attack * 18000.0) as i16;
    }
    make_wav(&s)
}
...
pub async fn load_sounds() -> Sounds {
    Sounds {
        ...
        explosion: load_sound_from_bytes(&gen_explosion_sound()).await.unwrap(),
...
pub fn play_events(events: &[u8], sounds: &Sounds) {
    for &e in events {
        match e {
            ...
            SND_EXPLOSION => play_sfx(&sounds.explosion, 0.7),
            _ => {}
        }
    }
}
